...
def test_stale_alert_fires_on_old_monotonic_stamp(monkeypatch):
    alerts = _stale_alerts(monkeypatch, time.monotonic() - 400.0)
    assert len(alerts) == 1
    assert alerts[0].alert_type == "STALE_BALANCE"


def test_fresh_monotonic_stamp_is_quiet(monkeypatch):
    assert _stale_alerts(monkeypatch, time.monotonic()) == []


def test_wall_clock_poisoned_stamp_blinds_the_watchdog(monkeypatch):
    # Documents WHY the handler pin above matters: an epoch stamp makes the
    # age diff hugely negative, so the watchdog stays silent no matter how
    # stale the balance really is (monotonic reaching epoch scale would need
    # ~55 years of uptime). The engine's TTL check misreads the same stamp as
    # fresh-forever — frozen live sizing with no alert.
